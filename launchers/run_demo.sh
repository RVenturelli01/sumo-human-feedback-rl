...
GRAD_CLIP_REW=1.0
IMPORTANCE_WEIGHTING="${IMPORTANCE_WEIGHTING:-false}"  # IMPORTANCE_WEIGHTING=false ./run_demo.sh -> uniform-weight ablation
N_ENSEMBLES=3
NET_ARCH_REW="[32,32]"
ACTIVATION_FN="tanh"
ALFA=1
...
"$PYTHON_BIN" scripts/test_demo_PPO.py \
    --config-path ../configs \
    --config-name test_demo_PPO \
    run.seed="$SEED" \
    run.output_dir="$OUTPUT_DIR" \
    wandb.entity="$ENTITY" \
    wandb.project="$PROJECT" \
    "wandb.tags=[iw_$IMPORTANCE_WEIGHTING]" \
    env.id="$ENV_ID" \
    env.n_envs="$N_ENVS" \
    ...
    algo.kwargs.reward_model_kwargs.alfa="$ALFA" \
    algo.kwargs.grad_clip_rew="$GRAD_CLIP_REW" \
    algo.kwargs.importance_weighting="$IMPORTANCE_WEIGHTING" \
